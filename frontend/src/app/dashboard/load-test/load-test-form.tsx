...
import { Activity, AlertCircle, Clock, Gauge, Server, Zap } from "lucide-react"
          ...
          <CardContent className="bg-transparent">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label>API URL</Label>
                  <Input readOnly value={selectedApi.url} className="bg-muted" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={cn("bg-muted font-medium", methodColors[selectedApi.method] || "text-foreground")}
                  />
                </div>
                ...
                {countdown !== null && (
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">{countdown}s remaining</span>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <Card>
                ...
                <CardContent className="space-y-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    <MetricCard title="Total Requests" value={result.data.requests} icon={Zap} />
                    <MetricCard
                      title="Success Rate"
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-medium text-muted-foreground">Detailed Latencies</h4>
                    <div className="grid gap-4 md:grid-cols-3">
                      {Object.entries(result.data.latencies)
                        .filter(([key]) => key !== "mean")
                        .map(([key, value]) => (
                          <div key={key} className="rounded-lg border bg-card p-3">
                            <div className="text-sm font-medium text-muted-foreground">{key.toUpperCase()}</div>
